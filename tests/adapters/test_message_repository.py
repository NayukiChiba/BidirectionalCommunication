...
@pytest.mark.asyncio
async def test_add_stores_message_through_application_port() -> None:
    """内存适配器应满足应用层定义的保存需求。"""
    repository = InMemoryMessageRepository()
    message = make_message()

    await useRepository(repository, message)

    assert repository.messages == (message,)


@pytest.mark.asyncio
async def test_messages_returns_snapshot() -> None:
    """读取快照后新增消息不应改变旧快照。"""
    repository = InMemoryMessageRepository()
    first_message = make_message("first")
    second_message = make_message("second")
    await repository.add(first_message)

    snapshot = repository.messages
    await repository.add(second_message)
...
@pytest.mark.asyncio
async def test_add_keeps_one_record_per_message_identity() -> None:
